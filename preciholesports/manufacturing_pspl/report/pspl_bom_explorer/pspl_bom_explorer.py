# Copyright (c) 2023, preciholesports and contributors
...
def execute(filters=None):
	...
	in_house_costing = frappe.get_value('BOM Record', filters.bom_record, 'in_house_costing')
	data.append(
		{
...
def get_data(filters, data):
	bom = frappe.get_value('BOM Record', filters.bom_record, 'bom')
	if bom:
...
def get_exploded_items(bom, parent, data, flag, bom_level):
	...
	for item in bom_items:
		item_dict = frappe.get_value('Item', item.item_code, ['last_purchase_rate', 'min_order_qty', 'lead_time_days', 'safety_stock', 'stock_uom', 'item_group', 'method_of_procurement', 'manufacturing_cost_c'], as_dict=1)
		...
		if frappe.get_value("Has Role",{"role":'Purchase Manager', 'parent': frappe.session.user}, 'role') == 'Purchase Manager':
			...
			if item.bom_no and (item.item_group == "Manufactured Components" and item.method_of_procurement == 'Manufacture'):
				...
				if frappe.get_value('Item', item.item_code, 'method_of_procurement') == 'Manufacture':
					#manufacturing_cost_c is custom field in item
					item.costing = round(frappe.get_value('Item', item.item_code, 'manufacturing_cost_c') * item.qty, 2)
				else:
					item.costing = round(frappe.get_value('Item', item.item_code, 'last_purchase_rate') * item.qty, 2)
		else:
			item.costing = 0
		data.append(item)
		if item.bom_no:
...
def get_suppliers_for_item(item_code):
	...
	unique_supplier = []
	supplier_string = ""
	purchase_orders = frappe.get_all('Purchase Order Item',
		{
			'item_code': item_code,
			'docstatus': 1
		},
	...
	for po in purchase_orders:
		supplier = frappe.get_value('Purchase Order', po.parent, 'supplier')
...
def calculate_item_cost(parent, parent_bom):
	...
	for row in bom_record_items:
		...
		mop = frappe.get_value('Item', row.item_code, 'method_of_procurement')
		if (row.bom_no and item_group == "Sub-assembly") or (row.bom_no and item_group == "Manufactured Components" and mop == "Manufacture"):
			...
			rm_cost = calculate_item_cost(parent, row.bom_no)
			cost = cost + rm_cost
			if frappe.get_value('Item', row.item_code, 'method_of_procurement') == 'Manufacture':
				rm_cost = frappe.get_value('Item', row.item_code, 'manufacturing_cost_c')
				rm_cost = round(rm_cost * row.qty, 2)
				cost = cost + rm_cost
			else:
				rm_cost = frappe.get_value('Item', row.item_code, 'last_purchase_rate')
				rm_cost = round(rm_cost * row.qty, 2)
				cost = cost + rm_cost
		else:
			if frappe.get_value('Item', row.item_code, 'method_of_procurement') == 'Manufacture':
