...
frappe.ui.form.on('BOM Record Item', {
	update: function(frm,cdt,cdn) {
        ...
        frappe.call({
            async:false,
            method: 'preciholesports.manufacturing_pspl.doctype.bom_record.bom_record.check_bom_status',
            args:{
        ...
        if(delete_items_flag == true){
            frappe.call({
                async:false,
                method: 'preciholesports.manufacturing_pspl.doctype.bom_record.bom_record.delete_by_new_bom',
                args:{
                    child_table_name : child.name,
        ...
        if(add_items_flag == true){
            frappe.call({
                async:false,
                method: 'preciholesports.manufacturing_pspl.doctype.bom_record.bom_record.add_by_new_bom',
                args:{
                    child_table_name : child.name,
                    item_code : child.item_code,
                    bom_no : child.bom_no,
